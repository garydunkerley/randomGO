...
// specified location
func isBranchGenesis(s []string, location int) bool {
	return mSymb[i] == "("
}
...
func getMove(s string) (move, error) {
	...
	if strings.Contains(head, "B") {
		...
		return move, errors.New("Error (growBranch): 
	...
	if len(nodeData) != 2 && nodeData != "" {
		...
		return _, errors.New("Error (procureNodeId): String data cannot correspond to NodeId")
...
func (root *treeNode) growBranch(mSymb []string, location int, nodeCounter int) (newRoots []treeNode, newBranchGeneses []int, nodeCounter int, branchChart map[int]treeNode) {
	...
	for z := range mSymb {
		...
		} else if mSymb.isBranchGenesis(z) {
			for s := range getBranchGeneses(z){
				newRoots = append(newRoots, root)
				newBranchGeneses = append(newBranchGeneses, s)
			}
		} else if mSymb.isBranchEnd(z) {
...
func buildTree(mSymb []string) map[int]treeNode, error {
	...
	for branchesToGrow {
		if len(roots) != len(branchGeneses) {
			return _, errors.New("Error (buildTree): len(roots) =/= len(branchGeneses) ")
		}
		newRoots, newGeneses, nodeCounter, chart = roots[0].growBranch(mSymb, branchGeneses[0], nodeCounter)
